...
Install xref:cli.adoc[upsilon-cli] and run bootstrap to publish the drone bootstrap script, fabric-config git repository, and `upsilon-drone` binary over HTTP. See xref:setup-controlnode.adoc[Control Node Bootstrap] for full details.

....

root@host: git clone https://github.com/upsilonproject/upsilon-cli.git
root@host: cd upsilon-cli && make && make install
root@host: upsilon context set default --controller-host upsilon --current
root@host: upsilon controlnode bootstrap --drone-binary /usr/local/sbin/upsilon-drone
